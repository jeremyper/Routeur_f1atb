...
void sendMQTTDiscoveryMsg_global() {
  ...
  if (SmaOn == 1) {
    ...
    DeviceToDiscover("Energie_Routee_Jour", "Energie Routee Jour", "Wh", "energy", "0");
...
void SendDataToHomeAssistant() {
  ...
  //Énergie détournée et économies : publiées sans condition, la mesure ne dépendant
  //plus d'un onduleur SMA.
  len += snprintf(value + len, sizeof(value) - len, ",\"Energie_Routee_Jour\":%.0f", EnergieRouteeJour);
